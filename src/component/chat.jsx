...
function Chat({ user, socket, whom, setWhom }) {
  ...
  useEffect(() => {
    if (whom) {
      ...
      const fetchMessage = async () => {
        try {
          const response = await axios.get(`http://localhost:3000/api/get?user1=${user._id}&user2=${whom._id}`);
  ...
  const handleMessageSend = async () => {
    if (message.trim() !== '' && whom) {
      ...
      try {
        await axios.post(`http://localhost:3000/api/send?user1=${user._id}&user2=${whom._id}&content=${message}&sender=${user._id}`);
        setMessages((prevMessages) => {
          const updatedMessages = [...prevMessages, newMessage];
          localStorage.setItem(`messages_${whom._id}`, JSON.stringify(updatedMessages));
          return updatedMessages;
