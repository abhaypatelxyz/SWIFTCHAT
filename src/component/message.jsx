...
const Message = ({ user, whom, setWhom, contact, setContact }) => {
            ...
            if (user.contact && user.contact.length > 0) {
                try {
                    const contactPromises = user.contact.map(async (_id) => {
                        const response = await axios.get(`http://localhost:3000/api/contactdata?_id=${_id}`);
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:3000/api/username?userName=${searchQuery}`);
    ...
    const handleSendFriendRequest = async (result) => {
        try {
            await axios.post(`http://localhost:3000/api/sendfriendrequest?sender=${user._id}&receiver=${result._id}`);
