            ...
            try {
                ...
                await Promise.all(
                    user.friendRequestSend.map(async (request) => {
                        const response = await axios.get(`http://localhost:3000/api/contactdata?_id=${request.receiver}`);
            ...
            try {
                ...
                await Promise.all(
                    user.friendRequestReceived.map(async (request) => {
                        const response = await axios.get(`http://localhost:3000/api/contactdata?_id=${request.sender}`);
